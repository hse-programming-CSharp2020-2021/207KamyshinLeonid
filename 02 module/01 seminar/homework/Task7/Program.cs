...
namespace Task7
{
    class Program
    {
        ...
        static int[,] auto = { { 20, 24, 25 }, { 21, 20, 18 }, { 23, 27, 26 }, { 22, 19, 20 } };
        ...
        public static string PrintResults(string mode)
        {
            ...
            int MaxAutoKvartal;
            // переключатель
            // продано лучшим филиалом за год
            // продано за квартал
            // номер квартала с максимальной продажей
            // максимальная продажа в квартал
            switch (mode)
            {
        ...
        private static void maxAutoFilialZaGod(out int Nfiliala_MaxAutoYear, out int MaxAutoFIlialZaGod)
        {
            ...
            for (int i = 0; i < auto.GetLength(1); i++)
            {
                ...
                if (tmpSum > MaxAutoFIlialZaGod)
                {
                    MaxAutoFIlialZaGod = tmpSum;
        ...
        private static void maxAutoKvartal(out int NKvartal_MaxAuto, out int MaxAutoKvartal)
        {
            ...
            for (int i = 0; i < auto.GetLength(0); i++)
            {
                ...
                if (tmpSum > MaxAutoKvartal)
                {
                    MaxAutoKvartal = tmpSum;
                    NKvartal_MaxAuto = i;
                }
            }
        }
